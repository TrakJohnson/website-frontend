  ...
  ngOnInit(): void {
    ...
    this.eventService.getOneEvent(this.event_id)
      .then((response) => {
        ...
        // participant infos
        this.getInfosParticipants().then(x => {
            this.usersInfos = Object.fromEntries(x.map((user: any) => [user.login, [user.prenom, user.nom]]));
    ...
    this.accountSub = this.acc.compte$.subscribe(
      (status) => {
        if (status) {
          ...
          this.is_already_claimed = !status?.placesClaimed.every(place => {
            return place.event_id != this.event_id
          });
        }
      });
