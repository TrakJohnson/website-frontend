...
export class EventService {
    ...
    createEvent(titre : string, description : string, date : Date, lieu : string, image: string, idPole : number, createur : string, dateOuverture : Date, dateFermeture : Date, nPlaces : number, prixC : number, prixNC : number, points : number) {
        return new Promise<any>((resolve, reject) => {
            ...
            {title : titre, description : description, dateEvent : date, event_place : lieu, thumbnail : image, pole_id : idPole, creator : createur, date_open : dateOuverture, date_close : dateFermeture, num_places : nPlaces, cost_contributor : prixC, cost_non_contributor : prixNC, points : points})
            .subscribe(
                (response) => {
                    resolve(response);
                },
